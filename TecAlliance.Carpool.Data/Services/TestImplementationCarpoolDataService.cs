...
namespace TecAlliance.Carpool.Data.Services
{
    ...
    {/*      private string carpoolPath = TecAlliance.Carpool.Data.Properties.Resources.CarpoolCsvPath;*/
        ...
        private string? path;
        string connectionString = @"Data Source =localhost; Initial Catalog = CarpoolApp;Integrated Security=True;";
        ...
        public void CreateNewCarpool(CarpoolEntity carpools, int userId)
        {
            ...
            {
                string queryString = $"INSERT INTO Carpool (CarpoolName, CarpoolOrigin, CarpoolDestination, CarpoolTime, CarpoolSeatCount,DriverId) VALUES('{carpools.CarpoolName}', '{carpools.Start}', '{carpools.Destination}', '{carpools.Time}', '{carpools.Seatcount}', @userId)";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    while (reader.Read())
                    {
                        ...
                        carpools.Start = reader["CarpoolOrigin"].ToString();
                        carpools.Destination = reader["CarpoolDestination"].ToString();
                        carpools.Time = reader["CarpoolTime"].ToString();
                        carpools.Seatcount = Convert.ToInt32(reader["CarpoolSeatCount"]);
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    while (reader.Read())
                    {
                        ...
                        carpoolToReturn.Start = reader["CarpoolOrigin"].ToString();
                        carpoolToReturn.Destination = reader["CarpoolDestination"].ToString();
                        carpoolToReturn.Destination = reader["CarpoolTime"].ToString();
                        carpoolToReturn.Seatcount = Convert.ToInt32(reader["CarpoolSeatCount"]);
                    }
                }
                finally
                {
        ...
        public void AddUserToCarpool(int carpoolId, int userId)
        {
            using(SqlConnection connection = new SqlConnection(connectionString))
            {
                string queryString = $"INSERT INTO CarpoolUser (CarpoolId, UserId) VALUES (@carpoolId, @userId)";
                SqlCommand command = new SqlCommand(queryString, connection);
        ...
        public void LeaveCarpool(int carpoolId, int userId)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string queryString = $"DELETE FROM CarpoolUser WHERE CarpoolUser.CarpoolId = @carpoolId AND CarpoolUser.UserId = @UserId";
                SqlCommand command = new SqlCommand(queryString, connection);
                command.Parameters.Add("@carpoolId", System.Data.SqlDbType.Int);
                command.Parameters["@carpoolId"].Value = carpoolId;
                command.Parameters.Add("@UserId", System.Data.SqlDbType.Int);
        ...
        public CarpoolEntity? ChangeCarpoolName(string carpoolName, int carpoolId)
        {
                ...
                string queryString = $" UPDATE Carpool SET CarpoolName = '{carpoolName}' WHERE CarpoolId = @carpoolId";
                    ...
                    try
                    {
                        ...
                        newCarpool.Time = reader["Destination"].ToString();
                            newCarpool.Seatcount = Convert.ToInt32(reader["CarpoolSeatCount"]);
                        }
                    }
                    finally
                    {
        ...
        public List<int> GetPassengerIds(int carpoolId)
        {
            ...
            {
                //SQL-Injection 
                string queryString = $"SELECT * FROM CarpoolUser WHERE CarpoolId = @carpoolId";
                SqlCommand command = new SqlCommand(queryString, connection);
                command.Parameters.Add("@carpoolId", System.Data.SqlDbType.Int);
                command.Parameters["@carpoolId"].Value = carpoolId;
                //SQL-Injection End
