...
namespace TecAlliance.Carpool.Data.Services
{
    public class PassengerDataService : IPassengerDataService
    {
        ...
        //public void AddNewDriverToCsv(string firstName, string lastName, string password)
        {
            ...
            {
                string queryString = $"INSERT INTO Users (UserFirstName,UserLastName,UserPassword) VALUES('{passenger.FirstName}', '{passenger.LastName}', '{passenger.Password}')";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    while (reader.Read())
                    {
                        ...
                        passengerToReturn.FirstName = reader["UserFirstName"].ToString();
                        passengerToReturn.LastName = reader["UserLastName"].ToString();
                    }
                }
                finally
                {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    while (reader.Read())
                    {
                        ...
                        foo.FirstName = reader["UserFirstName"].ToString();
                        foo.LastName = reader["UserLastName"].ToString();
